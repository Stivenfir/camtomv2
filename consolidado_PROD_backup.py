...
from xlsxprocesotiempos import xlsx_process, endpointminimas
from prodextractgeneral import ocr_factura

# Configuración básica del logging
logging.basicConfig(
    filename='prodprocesamiento_facturas.log',  # Archivo donde se guardará el log
...
def extraer_incoterm(incoterm_camtom):
    ...
    if conn:
        cursor = conn.cursor()
        cursor.execute(""" SELECT TOP 1 IncotermID
                            FROM BSIncoterm
                            WHERE UPPER(IncotermNombre) LIKE '%' + UPPER(?) + '%' """, incoterm_camtom)
        resultado = cursor.fetchone()
        cursor.close()
        conn.commit()

    print(f"#### {incoterm_camtom} ####")

    if resultado:
        incoterm_id = resultado[0]
        print(incoterm_id)
        return incoterm_id
...
def procesar_factura(archivo_path, clienteid, tipodoc):
    ...
    if not os.path.exists(archivo_path) or not os.access(archivo_path, os.R_OK):
        error_msg = f"❌ Error: El archivo {archivo_path} no se puede abrir o no existe."
        print(error_msg)
    ...
    try:
        ...
        error_msg = f"❌ Error consumiendo OCR/API: {str(e)}"
        print(error_msg)
        raise Exception(error_msg)
    ...
    if response.status_code == 200:
        ...
        fechafactura_str = jsonrespuesta['document_data']['invoice'].get('date', '')
        print(f"##### {fechafactura_str} #####")
        try:
            IAFAC_FechaFactura = fechafactura_str
        except Exception:
            IAFAC_FechaFactura = None

        fechaoc_str = jsonrespuesta['document_data']['purchase_order'].get('date', '')
        print(f"##### {fechaoc_str} #####")
        try:
            IAFAC_FechaOC = fechaoc_str
        except Exception:
            IAFAC_FechaOC = None
        ...
        data_factura = {
            "IAFAC_NumeroFactura": jsonrespuesta['document_data']['invoice'].get('id', ''),
            "IAFAC_FechaFactura": IAFAC_FechaFactura,
            "IAFAC_Incoterm": extraer_incoterm(jsonrespuesta['document_data']['invoice'].get('incoterm', '')),
            "IAFAC_Moneda": jsonrespuesta['document_data']['invoice'].get('currency', ''),
            "IAFAC_Importe": analizar_separador_decimal(jsonrespuesta['document_data']['invoice'].get('subtotal', SINDATOS)) if jsonrespuesta['document_data']['invoice'].get('subtotal', SINDATOS) == SINDATOS else jsonrespuesta['document_data']['invoice'].get('subtotal', SINDATOS),
            "IAFAC_Total": analizar_separador_decimal(jsonrespuesta['document_data']['invoice'].get('total', SINDATOS)) if jsonrespuesta['document_data']['invoice'].get('total', SINDATOS) == SINDATOS else jsonrespuesta['document_data']['invoice'].get('total', SINDATOS),
            "IAFAC_CostoFlete": analizar_separador_decimal(jsonrespuesta['document_data']['invoice'].get('freight_cost', SINDATOS)),
            "IAFAC_Seguro": analizar_separador_decimal(jsonrespuesta['document_data']['invoice'].get('insurance', SINDATOS)),

            "IAFAC_NumeroOC": jsonrespuesta['document_data']['purchase_order'].get('number', ''),
            "IAFAC_FechaOC": IAFAC_FechaOC,
            "IAFAC_PosicionOC": jsonrespuesta['document_data']['purchase_order'].get('position', SINDATOS),
            ...
            "IAFAC_RazonSocialProveedor": jsonrespuesta['document_data']['vendor'].get('address_recipient', SINDATOS),
        ...
        if itemslist:
            ...
            for item in itemslist:
                unit_price = item.get('unit_price', SINDATOS)
                amount = item.get('total_amount', SINDATOS)
                ...
                data_item = {
                    ...
                    "IAItemFAC_ImporteTotal": ImporteTotal if ImporteTotal == SINDATOS else analizar_separador_decimal(str(ImporteTotal)),
                    "IAItemFAC_Subtotal": Subtotal if Subtotal == SINDATOS else analizar_separador_decimal(str(Subtotal)),
                    "IAItemFAC_Total": (
                        item.get('subtotal') if amount == SINDATOS else analizar_separador_decimal(str(amount))
                    ),
                    "IAItemFAC_PesoBrutoKg": item.get('gross_weight_kg', SINDATOS),
                    "IAItemFAC_PesoNetoKg": item.get('net_weight_kg', SINDATOS)
                }
        ...
        print(f"entró al else: {error_msg}")
        raise Exception(error_msg)
    ...
    if conn:
        for item in items_factura:
            ...
            try:
                ...
                """, (
                    int(IAFAC_FacturaID),
                    item['IAItemFAC_NumeroOC'],
                    item['IAItemFAC_FechaOC'],
                    item['IAItemFAC_PosicionOC'],
                    item['IAItemFAC_Referencia'],
                    item['IAItemFAC_CodInterno'],
                    item['IAItemFAC_PaisOrigen'],
                    item['IAItemFAC_Marca'],
                    item['IAItemFAC_Descripcion'].replace("'", " "),
                    item['IAItemFAC_AnioFabricacion'],
                    item['IAItemFAC_Cantidad'],
                    item['IAItemFAC_Unidad'],
                    item['IAItemFAC_PrecioUnitario'],
                    item['IAItemFAC_ImporteTotal'],
                    item['IAItemFAC_Subtotal'],
                    item['IAItemFAC_Total'],
                    item['IAItemFAC_PesoBrutoKg'],
                    item['IAItemFAC_PesoNetoKg']
                ))
    ...
    if conn:
        ...
        if estadoprocesamientoia == 0:
            ...
            if encabezado_path: #and IAPR_ProcesarFacturaID: #procesarfacturaia
                ...
                ### cursor.execute("""UPDATE IA_IM_Factura SET FechaRecibidoDelCoordinador = ? WHERE IAPR_ProcesarFacturaID = ?""", (datetime.now(), IAPR_ProcesarFacturaID))
                ### conn.commit()
...
@app.get("/prodprocesoclasificacion/{docimpoid:path}")
async def prodprocesoclasificacion(docimpoid: str, background_tasks: BackgroundTasks):
...
async def procesoclasificacion_background(docimpoid: str):
    ...
    try:
        ...
        if conn:
            ...
            cursor.execute("""
                SELECT TRABAJO.IAFAC_FacturaID, TRABAJO.IAItemFAC_ItemFacID, TRABAJO.IAItemFAC_Descripcion 
            ...
            IAFAC_FacturaIDestado5 = id_y_descripcion[0][0]
            ...
            if itemsprocesados:
                for item in itemsprocesados:
                    ...
                    descminimasaplicables, descminimasobligatorias = endpointminimas(item.get('DESCRIPTION'), item.get('HS CODE'))
                    ...
                    params = (
                        ...
                        ', '.join(acuerdos_comerciales) if isinstance(acuerdos_comerciales, list) else acuerdos_comerciales if isinstance(acuerdos_comerciales, str) else SINDATOS,
                        IAItemFAC_ItemFacID
                    )

                    print("Parámetros para actualización:")
                    print(params)
                    ...
                    try:
                        ...
                        """, params)
                        conn.commit()
                        print((f"Actualización exitosa de IAItemFAC_ItemFacID {IAItemFAC_ItemFacID}"))
                        logging.info(f"Actualización exitosa de IAItemFAC_ItemFacID {IAItemFAC_ItemFacID}")
                    except Exception as e:
                        conn.rollback()
                        print(f"Error al actualizar IAItemFAC_ItemFacID {IAItemFAC_ItemFacID}: {e}")
        ...
        logging.critical(f"Error en proceso de clasificación para docimpoid {docimpoid}: {e} - {traceback.format_exc}")
        logging.debug(traceback.format_exc())
        if IAFAC_FacturaIDestado5:
            estado_procesamientoia(5, IAFAC_FacturaIDestado5)
            logging.error("Se actualizó estado procesamiento IA a estado ::: 5 :::")
        logging.error(":::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::")
                    ...
                    try:
                        ...
                        cursor.execute("""
                            UPDATE IA_IM_FacturaItem SET
                                IAItemFAC_Confiabilidad = ?, IAItemFAC_PosArancelID = ?, IAItemFAC_ResumenClasificacion = ?, justification = ?, recommendations = ?,
                                tariff_info_arancel = ?, tariff_info_arancel_var = ?, tariff_info_iva = ?, tariff_info_ucomercial_id = ?,
                                tariff_info_minimasaplicables = ?, tariff_info_minimasobligatorias = ?, requisitos_regimenlicenciaprevia = ?,
                                requisitos_cnestupefacientes = ?, requisitos_fnestupefacientes = ?, requisitos_icagropecuario = ?,
                                requisitos_descminima = ?, requisitos_industriamilitar = ?, requisitos_ingeominas = ?, requisitos_inpesca = ?,
                                requisitos_invmedyalimentos = ?, requisitos_minagricultura = ?, requisitos_mindesarrollo = ?, requisitos_minminas = ?,
                                requisitos_mintransporte = ?, requisitos_minmedioambiente = ?, requisitos_minsalud = ?, requisitos_supintvigyseguridad = ?,
                                requisitos_supintindycomercio = ?, requisitos_autopartesycomplementos = ?, requisitos_medidasantidumping = ?,
                                requisitos_observaciones = ?, requisitos_decreto1 = ?, requisitos_decreto2 = ?, tariff_info_acuerdoscomerciales = ?
                            WHERE IAItemFAC_ItemFacID = ?
                        """, (
                            float(item.get('CONFIDENCE')) if item.get('CONFIDENCE') is not None else 0,
                            str(item.get('HS CODE', SINDATOS)),
